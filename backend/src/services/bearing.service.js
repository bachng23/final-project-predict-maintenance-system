...
const getAllBearingsWithLatestStatus = async () => {
  ...
  return bearings.map((bearing) => {
    ...
    return {
      ...
      rul_minutes: latestPrediction.rulMinutes ?? null,
...
const getPredictionsByBearingId = async (bearingId, filters = {}) => {
  ...
  return predictions.map((p) => ({
    ...
    rul_minutes: p.rulMinutes ?? null,
    rul_lower_minutes: p.rulLowerMinutes ?? null,
    rul_upper_minutes: p.rulUpperMinutes ?? null,
    p_fail: p.pFail,
